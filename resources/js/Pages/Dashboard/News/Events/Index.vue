            ...
            <template v-for="(item, index) in events?.data" :key="index">
              <tr class="hover:bg-gray-100 border-b border-gray-200 py-2">
                ...
                <td class="px-4 py-2 flex items-center justify-center space-x-2">
                  <Badge  @click="confirmed(item)" class="bg-gray-600 text-white">
                    <i
...
export default {
  ...
  methods: {
    ...
    destroy() {
      this.$inertia.delete(route("new_event.destroy", this.event.id),{
        onError: errors => {cuteAlert({
            type: "error",
            title: "Error",
  ...
  watch: {
    form: {
      ...
      handler: throttle(function () {
        this.$inertia.get(route("new_event"), this.form, {
          preserveState: true,
        });
      }, 250),
    },
