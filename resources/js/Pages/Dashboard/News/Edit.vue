  ...
  data() {
    return {
      form: this.$inertia.form({
        ...
        title: this.novelty.translations.filter(item =>item.locale === 'es')[0].title,
        title_en:this.novelty.translations.filter(item =>item.locale === 'en')[0].title,
        content: this.novelty.translations.filter(item =>item.locale === 'es')[0].content,
        content_en: this.novelty.translations.filter(item =>item.locale === 'en')[0].content,
      }),
    };
  },
  methods: {
    update() {
      ...
      this.$inertia.put(route("new.update",this.novelty.id), data, {
        onSuccess: (page) => {},
        onError: (errors) => {
          console.log(errors);
        },
