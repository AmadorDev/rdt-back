...
export default {
    ...
    methods: {
        store() {
            ...
            formData.append("novelty_id", this.novelty.id);

            let config = { headers: { "Content-Type": "application/json" } };
            axios
                .post(
                    route("new_image.store", this.novelty.id),
                    formData,
                    config
                )
                .then((response) => {
        ...
        deleteImage(id) {
            Inertia.delete(route("new_image.destroy", id), {
                onSuccess: (page) => {},
                onError: (errors) => {
                    cuteAlert({
                        type: "error",
        ...
        setCover(id, val_origin, val) {
            ...
            Inertia.put(
                route("new_image.cover", id),
                { cover: val, novelty: this.novelty.id },
                {
                    onSuccess: (page) => {},
                    onError: (errors) => {
                        cuteAlert({
