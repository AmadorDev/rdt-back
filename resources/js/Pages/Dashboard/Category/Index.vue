  ...
  <AppLayout>
    ...
    <div class="pb-4 rounded-md w-full bg-white shadow-md p-5">
      <div class="flex justify-between w-full">
        <p class="py-2 uppercase">Categorias ({{ categories?.total }})</p>
      </div>
      <!-- search -->
      ...
      <div class="overflow-x-auto mt-6">
        <table class="table-auto border-collapse w-full">
          ...
          <tbody class="text-sm font-normal text-gray-700">
            <template v-for="(item, index) in categories?.data" :key="index">
        ...
        <Paginate :links="categories.links" :total="categories.total"></Paginate>
      </div>
    </div>
  </AppLayout>
  <JetConfirmationModal :show="showConfirmation">
    <template #title> Alerta! </template>
    <template #content> Estas seguro de Eliminar {{category.name}}? </template>
    <template #footer>
      <Button
...
export default {
  ...
  methods: {
    ...
    destroy() {
      this.$inertia.delete(route("category.destroy", this.category.id), {
        onError: (errors) => {
          cuteAlert({
            type: "error",
            title: "Error",
  ...
  watch: {
    form: {
      ...
      handler: throttle(function () {
        this.$inertia.get("/categories", this.form, {
          preserveState: true,
        });
      }, 250),
    },
