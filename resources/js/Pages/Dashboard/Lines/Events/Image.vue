<template>
  <AppLayout>
    <template #breadcrumb>
      <ItemCrumb title="Testing " :active="true" link="event"></ItemCrumb>
      <ItemCrumb title="Imagenes" :active="false" link=""></ItemCrumb>
    </template>

    <p class="text-center font-lg py-5 text-lg uppercase">Testing {{lineEvent?.name}}</p>
    <div class="flex md:flex-row md:space-x-2  flex-wrap justify-center">
...
export default {
  ...
  methods: {
    store() {
      ...
      axios
        .post(route('testing_image.store',this.lineEvent.id), formData, config)
        .then((response) => {
    ...
    deleteImage(id) {
      Inertia.delete(route("testing_image.destroy", id), {
        onSuccess: (page) => {},
        onError: (errors) => {
          cuteAlert({
            type: "error",
