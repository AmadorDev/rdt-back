            ...
            <template v-for="(item, index) in products?.data" :key="index">
              <tr class="hover:bg-gray-100 border-b border-gray-200 py-2">
                ...
                <td
                  ...
                  "
                >
                  <Link :href="route('product.image', item.id)">
...
export default {
  ...
  methods: {
    ...
    async showDialog(id) {
      try {
        const rs = await axios.get(route("product.image", id), {
          method: "GET",
        });
        this.modal = true;
    ...
    destroy() {
      this.$inertia.delete(route("product.destroy", this.product.id), {
        onError: (errors) => {
          cuteAlert({
            type: "error",
            title: "Error",
  ...
  watch: {
    form: {
      ...
      handler: throttle(function () {
        this.$inertia.get(route("product"), this.form, {
          preserveState: true,
        });
      }, 250),
    },
