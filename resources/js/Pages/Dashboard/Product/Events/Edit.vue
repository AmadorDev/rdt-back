<template>
  <AppLayout>
    <template #breadcrumb>
      <ItemCrumb title="Eventos" :active="true" link="product_event"></ItemCrumb>
...
export default {
  ...
  data() {
    return {
      form: this.$inertia.form({
        title: this.event.translations.filter((item) => item.locale === "es")[0]
          .title,
        title_en: this.event.translations.filter(
          (item) => item.locale === "en"
        )[0].title,
        content: this.event.translations.filter(
          (item) => item.locale === "es"
        )[0].content,
        content_en: this.event.translations.filter(
          (item) => item.locale === "en"
        )[0].content,
  ...
  methods: {
    update() {
      ...
      this.$inertia.post(route("product_event.update", this.event.id), data, {
        onSuccess: (page) => {},
        onError: (errors) => {
          console.log(errors);
        },
