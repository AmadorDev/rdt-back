  ...
  <AppLayout>
    <div class="pb-4 rounded-md w-full bg-white shadow-md p-5">
      <div class="flex justify-between w-full">
        <p class="py-2">Videos ({{ videos?.total }})</p>
      </div>
      <!-- search -->
      ...
      <div class="overflow-x-auto mt-6">
        <table class="table-auto border-collapse w-full">
          ...
          <tbody class="text-sm font-normal text-gray-700">
            <template v-for="(item, index) in videos?.data" :key="index">
              <tr class="hover:bg-gray-100 border-b border-gray-200 py-2">
                <td class="px-4 py-2">{{ item.title }}</td>
                <td class="px-4 py-2">{{ item.content.slice(0, 100) }}...</td>

                <td class="px-4 py-2 flex items-center space-x-2 justify-center">
                  <Badge
                  ...
                  <Link :href="route('product_video.edit', item.id)">
        ...
        <Paginate :links="videos.links" :total="videos.total"></Paginate>
      </div>
    </div>
    ...
    <JetConfirmationModal :show="showConfirmation">
      ...
      <template #content>
        Estas seguro de eliminar {{ video.title }} ?
      </template>
      <template #footer>
        <div class="space-x-2">
          <JetButton
...
export default {
  ...
  methods: {
    ...
    destroy() {
      this.$inertia.delete(route("product_video.destroy", this.video.id), {
        onError: (errors) => {
          cuteAlert({
            type: "error",
            title: "Error",
  ...
  watch: {
    form: {
      ...
      handler: throttle(function () {
        this.$inertia.get(route("product_video"), this.form, {
          preserveState: true,
        });
      }, 250),
    },
